...
inline void		_V_memset(void* dest, int fill, int count) { memset(dest, fill, count); }
inline void		V_memset(void* dest, int fill, int count) { memset(dest, fill, count); }
inline void		_V_memmove(void* dest, const void* src, int count) { memmove(dest, src, count); }
inline void		V_memcpy(void* dest, const void* src, int count) { memcpy(dest, src, count); }
inline int		_V_memcmp(const void* m1, const void* m2, int count) { return memcmp(m1, m2, count); }
inline int		_V_strlen(const char* str) { return (int)strlen(str); }
inline int		V_strlen(const char* str) { return (int)strlen(str); }
inline void		_V_strcpy(char* dest, const char* src) { strcpy(dest, src); }
inline int		_V_wcslen(const wchar_t* pwch) { return (int)wcslen(pwch); }
inline char*	_V_strrchr(const char* s, char c) { return (char*)strrchr(s, c); }
inline int		_V_strcmp(const char* s1, const char* s2) { return strcmp(s1, s2); }
